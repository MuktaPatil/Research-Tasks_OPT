...
## Cross-check: the period-by-period breakdown confirms 235, not 232

The PDF also prints a "Goals by Period" table (Q1-OT2) for Syracuse:
74 + 65 + 48 + 47 + 0 + 1 = **235**, matching the team-total row exactly
and independently confirming that 235 (not the 232 you get from summing
individual player rows) is the number to trust. This means the error is
very likely inside the player-level table specifically (one or more
players' goal counts are off), not in the team-total figures, which are
corroborated twice over. See `su_wlax_2025_goals_by_period.csv`.
